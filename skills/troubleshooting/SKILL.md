...
description: 系统性排查问题根因，通过代码分析和日志追踪定位故障。当用户遇到编译错误、运行时异常、单测失败、流水线报错、现网告警等需要定位问题时使用。
---
...
当用户提供的 log 不足以定位问题时，从代码中找关键 log 并提供 grep 命令：
...
**判断是否创建**：使用 `ask_followup_question` 询问用户问题类型：
- **流水线报错 / 现网告警** → 创建排查记录
- **开发调试中的问题** → 不创建，直接排查
...
| 规则 | 说明 |
|------|------|
| **Iron Law** | 禁止假设（结论必须基于代码+log）；未找到根因禁止提修复建议 |
| **主动找 log** | log 不足时，从代码中找关键 log 并提供 grep 命令 |
| **实时更新文档** | 每次新进展必须更新排查记录 |
| **3+ 轮规则** | 连续 3 轮假设被否定 → 扩大信息收集范围 |
