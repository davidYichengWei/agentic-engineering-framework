...
使用 `task` 调用 `test-writer` 子代理分析被测代码，提供以下信息：
...
`test-writer` 返回测试计划后，使用 `todo_write` 创建测试任务清单：
...
用户确认计划后，对每个测试任务使用 `task` 调用 `test-writer` 子代理生成测试代码：
...
3. **实际测试生成必须通过 `test-writer` 子代理**：不要在主 agent context 中直接写测试代码
...
| ❌ 错误做法 | ✅ 正确做法 |
|------------|-----------|
| 在主 agent 中直接写测试代码 | 通过 `test-writer` 子代理生成 |
